...
public class PrimeClientCD extends Thread {
    private static final String HOSTNAME = "localhost";
    private static final int PORT = 1234;
    private static final long INITIAL_VALUE = (long) 1e17;
    private static final Long COUNT = Long.valueOf(20);
    private static final String CLIENT_NAME = PrimeClientCD.class.getName();
    ...
    //is called only once
    public PrimeClientCD(String hostname, int port, long initialValue, long count, String requestMode) {
    ...
    public static void main(String args[]) throws IOException, ClassNotFoundException {
        ...
        while (!doExit) {
            ...
            if (requestMode.equals("DEFAULT") || requestMode.equals("POLLING")) {
                new PrimeClientCD(hostname, port, initialValue, count, requestMode).run();
            } else if (requestMode.equals("SYNC") || requestMode.equals("BOTH")) {
                for (int i = 0; i < 4; i++) {
                    new PrimeClientCD(hostname, port, initialValue, count, requestMode).start();
                }
            }
